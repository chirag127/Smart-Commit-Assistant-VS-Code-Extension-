...
async function generateCommitMessages(parsedDiff, tone, apiKey) {
    try {
        ...
        if (!response.ok) {
            throw new Error(data.error?.message || 'Failed to generate commit messages');
        }
        ...
        return [
            {
                ...
                explanation: 'Failed to generate specific commit message. Please check the diff manually.'
            }
        ];
    } catch (error) {
...
function extractFallbackSuggestions(text) {
    ...
    const suggestionRegex = /(\d+)\.\s*(?:message:|"message":)\s*"?([^"]+)"?\s*(?:explanation:|"explanation":)\s*"?([^"]+)"?/gi;
    
    let match;
    while ((match = suggestionRegex.exec(text)) !== null && suggestions.length < 3) {
        suggestions.push({
