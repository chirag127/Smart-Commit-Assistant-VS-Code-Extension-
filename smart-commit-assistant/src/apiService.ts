...
export class ApiService {
    private getApiEndpoint(): string {
        return vscode.workspace.getConfiguration('smartCommitAssistant').get('apiEndpoint') as string || 'http://localhost:3000';
    }

    private getApiKey(): string {
        return vscode.workspace.getConfiguration('smartCommitAssistant').get('apiKey') as string || '';
    }
     ...
     */
    public async generateCommitMessages(diff: string, tone: ToneType): Promise<CommitSuggestion[]> {
        try {
            const apiEndpoint = this.getApiEndpoint();
            const apiKey = this.getApiKey();

            const response = await axios.post(`${apiEndpoint}/generate-commit`, {
                diff,
                tone,
                apiKey
            });
